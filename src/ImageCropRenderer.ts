...
export function registerImageCropRenderer(plugin: ImageCropPluginInterface) {
  ...
  async function processEmbed(embed: HTMLElement) {
    ...
    // Add hover preview functionality if enabled in settings
  ...
  function runProcessing() {
    Array.from(document.querySelectorAll<HTMLElement>('.internal-embed.media-embed.image-embed.is-loaded')).forEach(embed => {
    ...
    for (const mutation of mutations) {
      ...
      for (const node of addedNodes) {
        if (node instanceof HTMLElement) {
          ...
          const embeds = Array.from(node.querySelectorAll<HTMLElement>('.internal-embed.media-embed.image-embed.is-loaded'));
